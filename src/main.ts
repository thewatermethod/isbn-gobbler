...
document.addEventListener("DOMContentLoaded", () => {
  ...
  sortForm.addEventListener("submit", (e: Event) => e.preventDefault());

  const setLoading = (loading: boolean) => {
    if (loading) {
      body.classList.add("loading");
      // bar.setAttribute('disabled', 'true');
      // csvButton.setAttribute('disabled', 'true');
      // sortSelect.setAttribute('disabled', 'true');
      // sortForm.setAttribute('disabled', 'true');
  ...
  const renderCSV = () => {
    ...
    window.open(
      encodeURI(
        `data:text/csv;charset=utf-8,title,author,dewey,isbn\n${csvString}`
      )
    );
  };
  ...
  const addEventListeners = (listItem: HTMLElement) => {
    ...
    if (removeButton) {
      removeButton.addEventListener("click", () => {
        ...
        if (isbn) {
          data.splice(
            data.findIndex((book) => book.isbn === isbn),
            1
          );
          renderBookData();
        }
  ...
  const renderBookData = () => {
    ...
    data.forEach((book) => {
      ...
      const author = listItem.querySelector(
        ".book-author"
  ...
  form.addEventListener("submit", async (e: Event) => {
    ...
    try {
      ...
      renderMessage("There was an error fetching your book. Please try again.");
    } finally {
      setLoading(false);
      // clear our form
      form.reset();
  ...
  sortSelect
    .querySelector(`option[value="${sortKey}"]`)
